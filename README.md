...
LIBRARIES_FILE=libraries.json PREFIX=/opt/imagemagick bash scripts/build.sh
...
1. `check-new-version.yml` detects a new upstream release → updates `libraries.json` → pushes tag `vX.Y.Z-N`
2. The tag push triggers `build.yml` → compiles on all 4 platforms → publishes a GitHub Release

**Bundled library update (Renovate):**
1. Renovate merges a PR updating library versions in `libraries.json`
...
1. `check-new-version.yml` が新しいアップストリームリリースを検出 → `libraries.json` を更新 → タグ `vX.Y.Z-N` をプッシュ
2. タグのプッシュにより `build.yml` がトリガー → 4プラットフォームでコンパイル → アーカイブ付きで GitHub Release を公開

**同梱ライブラリの更新（Renovate）:**
1. Renovate が `libraries.json` のライブラリバージョンを更新する PR をマージ
